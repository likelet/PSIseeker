...
//        System.out.println();
        ...
        if (args.length == 0) {
            ...
            System.out.println(ToolsforCMD.print_ansi_YELLOW("Calling sites with input library, Two bamfiles (index files as well) , genome reference and its index file( generated by 'samtools faidx') requaired for analysis :   \r\n\t\t")
            ...
            System.out.println(ToolsforCMD.print_ansi_YELLOW("Calling sites with input library(For PairedEnd reads), Two bamfiles (index files as well) , genome reference and its index file( generated by 'samtools faidx') requaired for analysis :   \r\n\t\t")
                    + ToolsforCMD.print_ansi_GREEN("java -jar PSIseeker.jar -runPE")
                    + ToolsforCMD.print_ansi_CYAN(" <TreatBam(sorted)> <ControlBan(sorted)> <genome> <outputfile> [optional parameters] "));

            System.out.println(ToolsforCMD.print_ansi_WHITE("Extra paramters for options"));
            System.out.println(ToolsforCMD.print_ansi_RED("\t\t-cov\t ")
        ...
        }else if (args[0].endsWith("-runPE")) {
            PSIseekerPairEnd psiseeker = new PSIseekerPairEnd(args[1], args[2],args[3]);
            if (FunctionClass.getArgsParameter(args, "-cov") != null) {
                psiseeker.filternumber = Integer.parseInt(FunctionClass.getArgsParameter(args, "-cov"));
                System.out.println(ToolsforCMD.print_ansi_GREEN(FunctionClass.getArgsParameter(args, "-cov")) + "  minimun reads threshold is applied for analysis");
            }
